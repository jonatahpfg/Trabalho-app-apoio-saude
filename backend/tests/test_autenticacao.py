...
@pytest.fixture
def gerenciador_com_usuario(gerenciador: GerenciadorDeUsuarios) -> GerenciadorDeUsuarios:
...
def test_autentica_usuario_com_credenciais_validas(gerenciador_com_usuario):
...
def test_rejeita_login_quando_senha_vazia(gerenciador_com_usuario):
    with pytest.raises(ErroDeValidacao):
        gerenciador_com_usuario.autenticar(email="ana@ubs.gov.br", senha="")
...
# técnicos (não diferenciamos "e-mail errado" de "senha errada").
# PLoP 2018: não usamos Catch Generic — capturamos CredenciaisInvalidas.
# ---------------------------------------------------------------------------

def test_rejeita_login_quando_email_nao_cadastrado(gerenciador_com_usuario):
...
def test_rejeita_login_quando_senha_incorreta(gerenciador_com_usuario):
    with pytest.raises(CredenciaisInvalidas):
        gerenciador_com_usuario.autenticar(
            email="ana@ubs.gov.br",
            senha="senha_errada",
...
def test_credenciais_invalidas_e_subtipo_de_erro_de_autenticacao(gerenciador_com_usuario):
...
def test_rejeita_login_quando_usuario_inativo(gerenciador_com_usuario):
    ...
    usuario_ativo = repositorio.buscar_por_email("ana@ubs.gov.br")
    repositorio.salvar(replace(usuario_ativo, ativo=False))
...
def test_usuario_inativo_e_subtipo_de_erro_de_autenticacao(gerenciador_com_usuario):
...
def test_usuario_inativo_nao_lanca_credenciais_invalidas(gerenciador_com_usuario):
