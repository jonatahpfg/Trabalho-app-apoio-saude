...
from gestao_usuarios.dominio.senha import ALGORITMO, ITERACOES, gerar_hash, verificar
from gestao_usuarios.dominio.usuario import Perfil


def test_hash_tem_formato_esperado():
    partes = gerar_hash("Senha123!").split("$")

    assert partes[0] == ALGORITMO
    assert int(partes[1]) == ITERACOES
    assert len(bytes.fromhex(partes[2])) == 16  # salt
    assert len(bytes.fromhex(partes[3])) == 32  # sha256


def test_verificar_aceita_senha_correta():
    assert verificar("Senha123!", gerar_hash("Senha123!")) is True


def test_verificar_rejeita_senha_incorreta():
    assert verificar("senha_errada", gerar_hash("Senha123!")) is False
...
def test_verificar_rejeita_hash_em_formato_invalido():
    assert verificar("Senha123!", "texto-que-nao-e-hash") is False
    assert verificar("Senha123!", "") is False
...
def test_usuario_persistido_nao_guarda_senha_em_texto_puro():
    ...
    assert usuario.senha_hash.startswith(f"{ALGORITMO}$")
