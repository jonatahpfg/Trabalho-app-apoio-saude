...
def adaptador(caminho_log) -> AdaptadorArquivoDeLog:
    return AdaptadorArquivoDeLog(
        ArquivoDeLogSimples(caminho_log)
    )
...
def _registro(
    ...
    return RegistroDeAcesso.criar(
        login=login,
        sucesso=sucesso,
        data_hora=datetime(2026, 7, 15, 10, 30),
    )


def test_salvar_atribui_ids_sequenciais(adaptador):
    primeiro = adaptador.salvar(
        _registro()
    )
    segundo = adaptador.salvar(
...
def test_buscar_todos_reconstroi_registros_salvos(adaptador):
    ...
    assert registros[0].data_hora == datetime(
        2026,
        7,
        15,
        10,
...
def test_registros_persistem_entre_instancias_do_adaptador(
    caminho_log,
):
    AdaptadorArquivoDeLog(
        ArquivoDeLogSimples(caminho_log)
    ...
    nova_instancia = AdaptadorArquivoDeLog(
        ArquivoDeLogSimples(caminho_log)
    )

    assert len(
        nova_instancia.buscar_todos()
    ) == 1


def test_buscar_todos_devolve_lista_vazia_quando_arquivo_nao_existe(
    adaptador,
):
    assert adaptador.buscar_todos() == []


def test_preserva_login_que_contem_o_separador(adaptador):
    adaptador.salvar(
        _registro(
            login="a;b"
        )
    ...
    assert registros[0].login == "a;b"


def test_lanca_erro_de_acesso_quando_linha_corrompida(
    caminho_log,
    adaptador,
):
    ArquivoDeLogSimples(
...
def test_lanca_erro_de_acesso_quando_nao_consegue_gravar(
    tmp_path,
):
...
def test_erro_de_acesso_e_subtipo_de_erro_de_persistencia(
    tmp_path,
):
