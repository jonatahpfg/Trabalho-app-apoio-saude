...
def test_registra_acesso_com_sucesso_quando_login_valido(
    gerenciador,
    repositorio_acessos,
):
...
def test_registra_falha_quando_senha_incorreta(
    gerenciador,
    repositorio_acessos,
):
...
def test_registra_falha_quando_login_nao_cadastrado(
    gerenciador,
    repositorio_acessos,
):
    with pytest.raises(CredenciaisInvalidas):
    ...
    assert len(registros) == 1
    assert registros[0].login == "inexistente"
    assert registros[0].sucesso is False


def test_registra_falha_quando_usuario_inativo(
    gerenciador,
    repositorio_acessos,
):
    repositorio = gerenciador._repositorio

    usuario = repositorio.buscar_por_login(
        "ana"
    )
...
def test_nao_registra_acesso_quando_campos_em_branco(
    gerenciador,
    repositorio_acessos,
):
...
def test_acumula_registros_de_varias_tentativas(
    gerenciador,
    repositorio_acessos,
):
    gerenciador.autenticar(
