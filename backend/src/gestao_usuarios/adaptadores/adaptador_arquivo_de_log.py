...
Formato de cada linha: ``id;sucesso;data_hora_iso;login`` (login por último
para tolerar logins que contenham o separador).
"""
    ...
    def salvar(self, registro: RegistroDeAcesso) -> RegistroDeAcesso:
        try:
            if registro.id is None:
                registro = replace(
                    registro,
                    id=len(self._arquivo_de_log.ler_linhas()) + 1,
                )

            self._arquivo_de_log.anotar(
            ...
            raise ErroDeAcessoAoArquivo(
                "Falha ao gravar registro de acesso no log."
            ) from erro
...
def _linha_para_registro(
    ...
    id_texto, sucesso_texto, data_hora_texto, login = partes
